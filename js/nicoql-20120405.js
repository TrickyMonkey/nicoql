/*!
 * NicoNico Quick Look Engine v12.0.1
...
function transportData() {
    ...
    $.ajax({
        ...
        data: {"type": type,
               "username": username,
               "password": password,
               "save_group1": localStorage.getItem("save_group1"),
               "save_group2": localStorage.getItem("save_group2"),
               "save_group3": localStorage.getItem("save_group3"), 
              },
...
function importDataProcess(data) {
    ...
    if (data["error"] === true) {
        ...
        // import savedata
        localStorage.setItem("save_group1", data["save_group1"]);
        localStorage.setItem("save_group2", data["save_group2"]);
        localStorage.setItem("save_group3", data["save_group3"]);
        return true;
    }
}
...
function exportDataProcess(data) {
     ...
     */
    if (data["error"] === true) {
        alert("他の人が既に同じユーザ名およびパスワードを使用しています。");
        return false;
    } else {
